...
if ($mday-4 < 0 )
{
$start = timelocal(0,0,0,28,$mon - 1,$year);
}
else
{
...
{
$delete = timelocal(0,0,0,$mday - 5,$mon,$year);
($mon, $mday, $year) = extractDate($delete);
print "deleteing at $mon/$mday/$year\n";
$deleted=`rm -rf $outputdir/year_$year/month_$mon/day_$mday`;
}
